...
def call_llm(
    ...
    - Never returns failure text silently
    ...
    # -------------------------------
    if not LLM_ENABLED:
        raise RuntimeError("LLM disabled by server configuration")

    now = time.time()
    if now - _LAST_FAILURE_TS < _FAILURE_COOLDOWN:
        raise RuntimeError("LLM in cooldown window")
